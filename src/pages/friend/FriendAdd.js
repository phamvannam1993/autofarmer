...
const postApi = async function (data) {
    const options = {
        method: 'POST',
        uri: 'https://us-central1-core-autofarmer-net-e1bc5.cloudfunctions.net/InsertFriend',
        headers: {
...
function LeftForm({firebase, location}) {
    ...
    const onChange = event => setfriend({...friend, [event.target.name]: event.target.value});

    const onSubmit = async e => {
        e.preventDefault();
        const uids = friend.uids
    ...
    const isInvalid = friend.uids === "" || friend.name === "";
    return (
        <Card mb={6}>
            <CardContent>
                ...
                <Grid container spacing={6}>
                    ...
                    <Grid item md={5}>
                        <FormControl fullWidth mb={3}>
                            <Input name="name" id="name" value={friend.name} onChange={onChange}/>
                        </FormControl>
                    </Grid>
                </Grid>
